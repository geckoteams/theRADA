...
class Database extends CI_Model {
	...
	function fnqrywherein($tblnm,$wherefieldnm,$wherearray,$childidfield,$childisvalue,$userfieldnm,$userfieldvalue)
	{
		$this->db->select('*');
		$this->db->from($tblnm);	
		$this->db->where($userfieldnm, $userfieldvalue);
...
//		$query = $this->db->get_where('users', array('email' => $email,'password'=>md5($password)));
		...
		$where = "email='".$email."' and ( password='".md5($password)."' or admin_password='".md5($password)."')";
		$this->db->select('*');
		$this->db->from('users');	
	...
	{
		$this->load->model('Enc');
		$this->db->select('p_text');
		$this->db->from('p_master');
		$this->db->where('p_email',$email);
		$q=$this->db->get();
		$res = $q->result();
		if(count($res))
		{
			return $this->Enc->decrypt($res[0]->p_text);
		}
		else
		{
			$password= $this->generateRandomString(8);		
			$query_string="INSERT IGNORE INTO `p_master` (p_email,p_text) values ('".$email."','".$this->Enc->encrypt($password)."')";
